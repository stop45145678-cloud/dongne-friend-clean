...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const payload = { banned: false, unbannedAt: new Date().toISOString() };
    await db.collection('profiles').doc(userId).set(payload, { merge: true });
    await db.collection('users').doc(userId).set(payload, { merge: true });

    return res.status(200).json({ success: true, message: '유저 정지 해제 완료', userId });
  } catch (error) {
    return res.status(500).json({ error: '유저 정지 해제 실패', detail: error.message });
  }
};
